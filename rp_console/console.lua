...
if radiant.is_server then
	rp.add_concommand({ 'openscript_server', 'os', 'oss' }, function(name, args, argstr)
		argstr = argstr:gsub('^[/\\.]+', ''):gsub('%..*', '')
		
		print(pcall(dofile, 'lua/' .. argstr .. '.lua'))
	end)
	...
	rp.add_concommand({ 'luas_run', 'lsr' }, function(name, args, argstr)
...
--[[ Client commands ]]--
	rp.add_concommand({ 'openscript_client', 'osc' }, function(name, args, argstr)
		argstr = argstr:gsub('^[/\\.]+', ''):gsub('%..*', '')
		
		print(pcall(dofile, 'lua/' .. argstr .. '.lua'))
	end)
	
	rp.add_concommand({'luac_run', 'lcr' }, function(name, args, argstr)
		local func, err = loadstring(argstr)
		if not func then
			rp.log(err)
		else
